import certifi, fitz, hashlib, json, lxml.html, math, pdftotext, requests
...
html = requests.get('https://stasis.net/daily-statements/')
html_xml = requests.get('https://stasis.net/weekly-statements/')
doc = lxml.html.fromstring(html.content)
doc_2 = lxml.html.fromstring(html_xml.content)
table_rows = doc.xpath('//table[@class="daily-statement__table"]/tr')
table_rows2 = doc_2.xpath('//table[@class="daily-statement__table"]/tr')

number_of_rows = len(table_rows)
number_of_rows2 = len(table_rows2)

daily_date_array = []
weekly_date_array = []

daily_pdf_array = doc.xpath('//table[@class="daily-statement__table"]/tr//td[@class="daily-statement__table-cell daily-statement__table-cell_download"]/a/@href')
weekly_pdf_array = doc_2.xpath('//table[@class="daily-statement__table"]/tr//td[@class="daily-statement__table-cell daily-statement__table-cell_download"]/a/@href')

tiny_pdf_array = []
tiny_pdf_array2 = []

weekly_files_downloaded = []
daily_files_downloaded = []

daily_pdf_count = len(daily_pdf_array)
print(f'Number of Daily PDF Statements: {certain_cyan + str(daily_pdf_count) + end_formatting}\n')
weekly_pdf_count = len(weekly_pdf_array)
print(f'Number of Weekly PDF Verifications: {certain_cyan + str(weekly_pdf_count) + end_formatting}\n')

#appends daily pdf statement urls into array
i = 0
while i < daily_pdf_count:
	row_line = table_rows[i].xpath("td")
	date_memory_index = row_line[0]
	date_string = date_memory_index.text_content()
	orgranized_date = date_string.split()[0][:-1] + " " + date_string.split()[1][:-1] + " " + date_string.split()[2]
	orgranized_datetime = datetime.strptime(orgranized_date, '%b %d %Y').date()
	daily_date_array.append(orgranized_datetime)
	i = i+1

#appends weekly pdf verification urls into array
j = 0
while j < weekly_pdf_count:
	row_line2 = table_rows[j].xpath("td")
	date_memory_index2 = row_line2[0]
	date_string2 = date_memory_index2.text_content()
	orgranized_date2 = date_string2.split()[0][:-1] + " " + date_string2.split()[1][:-1] + " " + date_string2.split()[2]
	orgranized_datetime2 = datetime.strptime(orgranized_date2, '%b %d %Y').date()
	weekly_date_array.append(orgranized_datetime2)
	j = j+1


shortener = Shortener('Tinyurl')

#converts long urls of daily statements to manageable text chunks
p = 0
while p < daily_pdf_count:
	url = daily_pdf_array[p]
	print (url)
	url_short = shortener.short(url)
	tiny_pdf_array.append(url_short)
	print(f'Daily Statement TinyURL >> {dirty_yellow + tiny_pdf_array[p] + end_formatting}')
	print(f'Daily Statement Count: {dirty_yellow + str((len(tiny_pdf_array))) + end_formatting}\n')
	p = p+1
	break	

#converts long urls of weekly verification to manageable text chunks	
r = 0
while r < weekly_pdf_count:
	url2 = weekly_pdf_array[r]
	print (url2)
	url_short2 = shortener.short(url2)
	tiny_pdf_array2.append(url_short2)
	print(f'Weekly Verification TinyURL >> {certain_cyan + tiny_pdf_array2[r] + end_formatting}')
	print(f'Weekly Verification Count: {certain_cyan + str((len(tiny_pdf_array2))) + end_formatting}\n')
	r = r+1
	#break
	
#Downloads daily PDFs from Stasis Daily Statements Archive
tiny_daily_url = URL(tiny_pdf_array[0])
f = open('daily_statement.pdf', 'wb')
f.write(tiny_daily_url.download(cached=False))
f.close()


#Loops through and dowloads all weekly PDFs from Stasis Weekly Verification Archive
for w in range(weekly_pdf_count):
	tiny_weekly_url = URL(tiny_pdf_array2[w])
	weekly_filename = "bdo_weekly" + str(w) + ".pdf"
	f = open(weekly_filename, 'wb')
	f.write(tiny_weekly_url.download(cached=False))
	f.close()
	weekly_files_downloaded.append(weekly_filename)
